...
exports.update = function (req, res) {
    ...
    User.findOneAndUpdate({ username: username }, updateUser, { new: true }, (err, result) => { //{ new: true } to replace the data with the updated, else they will be added to the document together with the old data
        if (err) {
            res.status(400).json({ status: false, data: err });
